...
const Player = () => {
  ...
  const {tract,seekBar,seekBg,playStatus,play,pause,time  } = useContext(PlayerContext);
  return (
    <div className='h-[10%] bg-black flex justly-between item-center text-white px-4'>
      <div className='hidden lg:flex items-center gap-4'>
        <img className='w-12' src={tract.image} alt=""/>
        <div>
          <p>{tract.name}</p>
          <p>{tract.desc.slice(0,12)}</p>
        </div>
      </div>
      ...
      <div className='flex items-center gap-5'>
        ...
        <div ref={seekBg} className='w-[60vw] max-w-[500px] bg-gray-300 rounded-full cursor-pointer'>
          <hr ref={seekBar} className='h-1 border-none w-0 bg-orange-800 rounded-full'/>
        </div>  
        <div>
